...
public class AuthorATS {
    ...
    public AuthorATS(int t, int aspect, Double a0, Double a1, Double b0, Double b1, Double c, Double d, Double[] e,
            Double f, Double lp) {
   ...
   public void doSampling(boolean flag) {
    ...
    public void sample(int c, boolean flag) {
        ...
        int e = docEntities.get(d);
            ...
            int newA = sampleNewAspectForSentence(d, sent, visitedAspects);
            for (int n = 0; n < words.length; n++) {
                int w = words[n];
                int t = topics[n];
                int s = sentiments[n];
            ...
            if (flag && newA != lastAspect && lastAspect != -1) { //author has moved on to a new aspect for the sentence
    ...
    public int sampleNewAspectForSentence(int d, Sentence sent, ArrayList<Integer> visitedAspects) {
        ...
        for(int i=0;i<A;i++){
            if(visitedAspects.contains(i))
                ...
                p[i] = (nDA[d][i] + alpha3) / (docLength.get(d) + alpha3Norm);
        }
        //approximate likelihood to avoid time intensive gamma sampling
        for (int n = 0; n < words.length; n++) {
            for (int i = 0; i < A; i++) {
                if (levels[n] != 0) {
                    p[i] *= (nASTW[i][sentiments[n]][topics[n]][words[n]] + alpha6);
                }
            }
        }
        for(int i=0;i<A;i++){
            for (int n = 0; n < words.length; n++) {
                if (levels[n] != 0) {
                    p[i] = p[i]/(nAST[i][sentiments[n]][topics[n]] + alpha6Norm+n);
                }
            }
        }
        for (int i = 0; i < A; i++) {
    ...
    private int sampleNewTopic(int d, int w, int l1, int a, int s) {
     ...
     private int sampleNewSentiment(int e, int u,int d, int w, int l1, int a, int t) {
        ...
        for (int i = 0; i < S; i++) {
            ...
            p[i] = (0.5*priorEntity+0.5*priorAuth)* likelihood;
            pTotal += p[i];
        }
        }
        Random r = new Random();
        ...
        for (Entity e : inputEntities) {
            ...
            for (Review r : reviews) {
                ...
                for (String contentSentence : contentSentences) {
                    String line = contentSentence.toLowerCase().replace(',', ' ').replace('.', ' ');
                    line = Helper.normalizeSentence(contentSentence, stopwords);
